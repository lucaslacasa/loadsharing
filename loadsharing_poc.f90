...
!
! Comments
! 1) WE ARE ASSUMING A NEIGHBORHOOD K=5 FOR EACH REGION/HOSPITAL/TRUST, THUS WE HAVE A 5-REGULAR GEOMETRIC GRAPH
...
integer, allocatable :: changes(:,:) !records, for each realisation, the destination for each node
integer :: total_num_regions !total number of regions/hospitals/trusts
integer, allocatable :: loadshare(:,:) !records, for each realisation, the total load shared (if none, =0)
...
call sharing_availability(i,ICU_demand,available, neighborhood(i,:,:), destination,loadshare(config,i),d_max)
                else
call sharing_availability(i,updated_ICU_demand(config,:),available, neighborhood(i,:,:), destination,loadshare(config,i),d_max)
